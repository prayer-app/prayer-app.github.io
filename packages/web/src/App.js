...
import { 
  ...
  resetDatabase, 
  exportData 
} from './utils/webStorage.js';
import { generateUUID, formatDateForStorage } from './utils/webUtils.js';
import notificationService from './utils/webNotifications.js';
import { 
  isElectron, 
  exportDataDesktop, 
  importDataDesktop, 
...
function App() {
  ...
  useEffect(() => {
    ...
    // Setup desktop listeners if running in Electron
    if (isElectron()) {
      setupDesktopListeners(handleExportData, handleImportData);
    }
  ...
  const handleExportData = useCallback(async () => {
    try {
      if (isElectron()) {
        const result = await exportDataDesktop();
        if (result.success) {
  ...
  const handleImportData = useCallback(async (data) => {
    try {
      if (isElectron()) {
        const result = await importDataDesktop(data);
        if (result.success) {
